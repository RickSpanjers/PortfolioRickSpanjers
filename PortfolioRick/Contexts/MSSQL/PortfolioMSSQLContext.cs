...
{
	public class PortfolioMSSQLContext :ConnectionHelper, IPortfolio
	{
		public bool CreateNewItem(PortfolioItem b)
		{
			MySqlConnection cnn = ReturnMYSQLConnection();
			string query = "INSERT INTO PortfolioItem (Name, Description, FrontpageImg, CategoryName, ItemTitle, CreatedDate, UrlTitle, UrlLink) VALUES (@Name, @Description, @FrontpageImg, @CategoryName, @ItemTitle, @CreatedDate, @UrlTitle, @UrlLink)";
		...
		public bool UpdateItemById(PortfolioItem b)
		{
			...
			{
				MySqlConnection cnn = ReturnMYSQLConnection();
				var query = "UPDATE PortfolioItem SET Name=@Name, Description=@Description, FrontpageImg=@FrontpageImg, CategoryName=@CategoryName, ItemTitle=@ItemTitle, CreatedDate=@CreatedDate, UrlTitle=@UrlTitle, UrlLink=@UrlLink WHERE ID = @Id";
				MySqlCommand newCmd = CreateMYSQLCommandText(query, cnn);
